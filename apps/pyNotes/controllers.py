...
def get_posts():
    result = []
    posts = db(db.post.is_reply == None and db.post.get('email') == auth.current_user.get('email')).select(orderby=~db.post.post_date).as_list()
    for post in posts:
        if post.get('starred') is True:
            result.append(post)
    for post in posts:
        if post.get('starred') is not True:
            result.append(post)
...
def save_post():
    ...
    db.post.update_or_insert(
        (((db.post.id == id) & (id is not None)) or (db.post.color != color) or (db.post.title != title) or (db.post.starred != starred)),
        id=id,
        content=content,
        is_reply=is_reply,
        title = title,
    ...
    return dict(content=content, id=id, is_reply=is_reply, color=color, title=title, starred = starred, post_date=post_date)
